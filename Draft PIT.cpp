...
class StudentInformationSystem {
	private:
		...
		void check_duplicate_records() {
		    ...
		    // Create a set to store unique duplicate records
		    ...
		    for (size_t i = 0; i < students.size(); ++i) {
		        for (size_t j = i + 1; j < students.size(); ++j) {
		            ...
		            if (students[i]->name == students[j]->name || students[i]->id == students[j]->id) {
		                // If a duplicate is found, add the index of both records to the duplicateIndices set
		                duplicateIndices.insert(i);
		                duplicateIndices.insert(j);
		    ...
		    if (foundDuplicates) {
		        ...
		        set<int>::iterator it = duplicateIndices.begin();
		        int idxCounter = 1;
		        while (it != duplicateIndices.end()) {
		            int idx = *it;
		            cout << "- DUPLICATE RECORD [" << idxCounter << "]" << endl << endl;
		            cout << *students[idx] << endl;
		            ++it;
		            ++idxCounter;
		        }
		        ...
		        if (chosenIndex >= 1 && chosenIndex <= duplicateIndices.size()) {
		            set<int>::iterator it = duplicateIndices.begin();
		            advance(it, chosenIndex - 1);
		            int chosenRecordIndex = *it;
	...
	public:
	    StudentInformationSystem() : adminU("admin"), adminP("12345"), undergradFilename("undergraduates.txt"), gradFilename("graduates.txt") {}
